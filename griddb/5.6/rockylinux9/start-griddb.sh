...
join_cluster() {
	sleep 5
	yyy=$(gs_stat -u $GRIDDB_USERNAME/$GRIDDB_PASSWORD| grep clusterStatus)
	until [ -n "$yyy" ]; do
		sleep 5
		yyy=$(gs_stat -u $GRIDDB_USERNAME/$GRIDDB_PASSWORD| grep clusterStatus)
	done
	gs_joincluster -u $GRIDDB_USERNAME/$GRIDDB_PASSWORD -c $GRIDDB_CLUSTER_NAME -w
	clusterStatus=$(gs_stat -u $GRIDDB_USERNAME/$GRIDDB_PASSWORD | jq '.cluster.clusterStatus')
	until [ $clusterStatus = "\"MASTER\"" ]; do
		sleep 5
		gs_joincluster -u $GRIDDB_USERNAME/$GRIDDB_PASSWORD -c $GRIDDB_CLUSTER_NAME -w
		clusterStatus=$(gs_stat -u $GRIDDB_USERNAME/$GRIDDB_PASSWORD | jq '.cluster.clusterStatus')
	done
...
if [ "${1}" = 'griddb' ]; then
    ...
    if [ $isSystemInitialized = 0 ]; then
        ...
        export GRIDDB_SLEEP_TIME=${GRIDDB_SLEEP_TIME:-"5"}
    ...
    join_cluster & exec gsserver --conf ./conf
fi
exec "$@"
